...
public class TaskDaoImpl implements TaskDao {
    ...
    public List<Task> findAllOrderByDate() {
        //query
        TypedQuery<Task> query = entityManager.createQuery("from Task order by dateCompleted desc,id desc",Task.class);
...
//        TypedQuery<Task> query = entityManager.createQuery("from Task where task_group_id=:groupId",Task.class);
        TypedQuery<Task> query = entityManager.createQuery("select t from Task t JOIN FETCH t.taskGroup tg where tg.id=:groupId order by t.dateCompleted desc,t.id desc",Task.class);
